...
// Copyright (c) 2017-2018 The Hashimocoin developers
...
const string strMessageMagic = "Hashimocoin Signed Message:\n";
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees)
{
	...
	if (pindexBest->nHeight == 1)
		nSubsidy = 2000000 * COIN;  // 2% Premine
	else if (pindexBest->nHeight <= FAIR_LAUNCH_BLOCK) // Block 5000, Instamine prevention
        nSubsidy = 20 * COIN/2;
	else if (pindexBest->nHeight <= 25000) // Block 25,000
		nSubsidy = 255 * COIN;
	else if (pindexBest->nHeight <= 50000) // Block 50,000
		nSubsidy = 217 * COIN;
	else if (pindexBest->nHeight <= 100000) // Block 100,000
		nSubsidy = 185 * COIN;
	else if (pindexBest->nHeight <= 500000) // Block 500,000 ~ 3m HSC (33% will go to hybrid masternodes)
		nSubsidy = 157 * COIN;
	else if (pindexBest->nHeight <= 1000000) // Block 1,000,000
		nSubsidy = 133 * COIN;
	else if (pindexBest->nHeight <= 1500000) // Block 1,500,000
		nSubsidy = 113 * COIN;
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees)
{
    ...
    nRewardCoinYear = COIN_YEAR_REWARD; // 0.20 20%
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    */
    ...
    // ----------- masternode payments -----------
    // Once upon a time, People were really interested in HSC.
    // So much so, People wanted to bring HSC to the moon. Even Mars, Sooner than the roadster...
    ...
    // People wanted masternodes... Really Bad. But King Carsen was already busy with the rest of HSC
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // Hashimocoin: ask for pending sync-checkpoint if any
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        strMiscWarning = strMessage;
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Hashimocoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Can this program work yet";
        CTransaction txNew;
        txNew.nTime = 1526701948;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x90d61234baa2b0749da9b8a89cee340a47cd40debb3fa1c31363670b1d80b579"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfe, 0xf6, 0xe8, 0xba };
